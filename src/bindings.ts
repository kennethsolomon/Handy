...
// NOTE: Manual edits were made for multi-language support (selected_languages, custom_initial_prompt).
// Re-run `tauri-specta` generation in debug mode to regenerate this file if needed.
...
async changeSelectedLanguagesSetting(languages: string[]) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_selected_languages_setting", { languages }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async changeCustomInitialPromptSetting(prompt: string | null) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_custom_initial_prompt_setting", { prompt }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
